...
def atomic(token, token_iter, token_list):
    if token.tag == ID:
        return ([("ID", token.symbol), "atomic"], token_iter)
    if token.tag == INTEGER:
        return ([("INTEGER", token.symbol), "atomic"], token_iter)
    else:
        return (None, token_iter)
...
def parse(tokens, partial=False):
    ...
    # Only return the parse tree if there was no syntax error.
    # Otherwise return None.
    return token_iter if parse_error else parse_tree
